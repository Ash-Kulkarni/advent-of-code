pub fn process_part_1(input: &str) -> i32 {
    let data: Vec<Vec<i32>> = input.lines().map(|line| {
        line.split(":").nth(1).unwrap()
            .split_whitespace().map(|n| n.parse::<i32>().unwrap()).collect()
    }).collect();
    ...
    times.iter().zip(distances.iter()).map(|(&time, &distance_to_beat)| {
        ...
        possible_distances.iter().filter(|&&d| d>distance_to_beat).count() as i32
